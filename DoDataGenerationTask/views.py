...
def first(request):
    return render(request, 'MyDataGenerationTasks.html', {'tasks': Task.objects.all()}, )
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if len(data_generation) > 0:
                ...
                return render(request, 'DataGenerationTask.html', {'data_instance_available': True,
                                                                   'task_object': Task.objects.get(id=task_id),
                                                                   'data_instance': data_instance,
                                                                   'user_id': user_id,
                                                                   'task_id': task_id}, )
            else:
                return render(request, 'DataGenerationTask.html', {'data_instance_available': False,
                                                                   'task_object': Task.objects.get(id=task_id),
                                                                   'task_id': task_id})
        except:
...
def view_my_generations(request):
    try:
        ...
        if len(generated_data_instances) > 0:
            return render(request, 'ViewMyGeneratations.html', {'generated_data_instances_available': True,
                                                               'task_object': Task.objects.get(id=task_id),
                                                               'generated_data_instances':generated_data_instances},)
        else:
            return render(request, 'ViewMyGeneratations.html', {'generated_data_instances_available': False,
                                                               'task_object': Task.objects.get(id=task_id), })
    except:
        return redirect('/DoDataGenerationTask/')
...
def view_my_generations_change(request):
    if request.method == 'POST':
        ...
        return render(request, 'ViewMyGenerationsChange.html',{'generated_data_instance':generated_data_instance,
                                                               'task_object': Task.objects.get(id=task_id),})
...
def view_my_generations_delete(request):
    ...
    try:
        ...
        return render(request, 'ViewMyGenerationsDelete.html', {'generated_data_instance_id': generated_data_instance_id})
