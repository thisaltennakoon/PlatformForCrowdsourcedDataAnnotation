...
def first(request):
    ...
    return render(request,"viewtask.html",{'user':user})

def Imageanalyse(request,TaskID):
    lables=[]
    classid =[]
    t = Task.objects.all()
    for t1 in t:
    ...
    data = {'labels': lables,'data': m, 'r':noOdata, 'k':r[0],'f':r[1],'a':r[2],'s':r[3], 'process':process}
    return render(request, 'Imagedataanalyse.html', data )
